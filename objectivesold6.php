  ...
  <div class="page-body">
    ...
    <div class="card mb-3"><div class="card-body py-2">
      <form method="GET" class="row g-2 align-items-center">
        ...
        <?php endif; ?>
        <!-- Search box — client-side, filters visible cards instantly -->
        <div class="col-auto ms-auto">
          <div class="input-group input-group-sm">
            <span class="input-group-text bg-transparent border-end-0"><i class="bi bi-search text-muted"></i></span>
            <input type="text" id="objSearch" class="form-control border-start-0"
                   placeholder="Search objectives…" style="min-width:180px"
                   oninput="filterAndPage()">
          </div>
        </div>
        <div class="col-auto text-muted small fw-600" id="objCount"><?= count($objectives) ?> objective<?= count($objectives)!==1?'s':'' ?></div>
      </form>
    </div></div>
    ...
    <div class="okr-card" id="obj-<?= $obj['id'] ?>">
      <div class="okr-card-header">
        <div class="flex-grow-1">
          ...
          <div class="okr-meta">
            <span>
              <i class="bi bi-calendar3"></i> <?= htmlspecialchars($obj['time_period']) ?> 
              <?php if ($obj['start_date'] || $obj['end_date']): ?>
                <span style="color:var(--border2);margin:0 .2rem">·</span>
                <?php if ($obj['start_date']): ?>
                  <?= date('d M Y', strtotime($obj['start_date'])) ?>
                <?php endif; ?>
                <?php if ($obj['start_date'] && $obj['end_date']): ?>
                  <i class="bi bi-arrow-right" style="font-size:.65rem;vertical-align:middle"></i>
                <?php endif; ?>
                <?php if ($obj['end_date']): ?>
                  <?= date('d M Y', strtotime($obj['end_date'])) ?>
                <?php endif; ?>
              <?php endif; ?>
            </span>
    ...
    <!-- Pagination -->
    <div id="paginationWrap" class="d-flex align-items-center justify-content-between mt-3" style="display:none!important">
        ...
        <div class="row g-3">
          <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-1">
              ...
              <small class="text-muted" id="krTitleCount">0 / 255</small>
            </div>
            <input type="text" name="title" id="krTitle" class="form-control" required
                   maxlength="255" placeholder="e.g. Reach 500 beneficiaries"
                   oninput="charCount('krTitle','krTitleCount',255)">
          </div>
          <div class="col-12">
            <div class="d-flex justify-content-between align-items-center mb-1">
              <label class="form-label mb-0">Description</label>
              <small class="text-muted" id="krDescCount">0 / 500</small>
            </div>
            <textarea name="description" id="krDesc" class="form-control" rows="2"
                      maxlength="500" placeholder="What does this key result measure?"
                      oninput="charCount('krDesc','krDescCount',500)"></textarea>
          </div>
          ...
          <div class="col-md-6">
            <div class="d-flex justify-content-between align-items-center mb-1">
              <label class="form-label mb-0">Unit <span class="text-muted small">(optional)</span></label>
              <small class="text-muted" id="krUnitCount">0 / 50</small>
            </div>
            <input type="text" name="unit" id="krUnit" class="form-control"
                   maxlength="50" placeholder="e.g. users, £"
                   oninput="charCount('krUnit','krUnitCount',50)">
          </div>
          <div class="col-md-6">
            <label class="form-label">Starting Value</label>
            <input type="number" name="current_value" id="krCurrent" class="form-control"
                   value="0" step="any" min="0" oninput="validateKRValues()">
          </div>
          <div class="col-md-6">
            <label class="form-label">Target Value <span class="text-danger">*</span></label>
            <input type="number" name="target_value" id="krTarget" class="form-control"
                   value="100" step="any" min="0.01" required oninput="validateKRValues()">
          </div>
          <?php if ($role==='Manager'): ?>
      ...
      <form id="progressForm">
        ...
        <div class="mb-3">
          ...
          <div class="input-group">
            <input type="number" name="new_value" id="progValue" class="form-control"
                   step="any" min="0" required
                   oninput="updateProgPreview()">
            <span class="input-group-text" id="progTarget">/ 100</span>
          </div>
...
function applyTypeRules(type) {
  ...
  // Helpers for owner control
  const grpOwner    = document.getElementById('grp-owner');
  const objOwnerSel = document.getElementById('objOwner');
  const objOwnerAuto= document.getElementById('objOwnerAuto');

  function showOwnerDropdown(visible) {
    if (grpOwner)     grpOwner.style.display = visible ? '' : 'none';
    if (objOwnerAuto) objOwnerAuto.disabled  = visible;   // disable auto hidden when dropdown visible
    if (objOwnerSel)  objOwnerSel.disabled   = !visible;  // disable dropdown select when hidden
    if (visible) {
      if (objOwnerSel)  objOwnerSel.setAttribute('required', '');
      if (objOwnerAuto) objOwnerAuto.removeAttribute('required');
    } else {
      if (objOwnerSel)  objOwnerSel.removeAttribute('required');
      if (objOwnerAuto) {
        objOwnerAuto.value = ME;  // always auto-set to current user for Org/Personal
      }
    }
  }

  if (type === 'Organisational') {
    // Auto-ownership: current user is always owner
    ...
    // Owner dropdown shown and required — must be a team member
...
function renderDetail(d){
  ...
  document.getElementById('detailBody').innerHTML=`
    ...
    <div class="row g-2 detail-sec">
      ...
      ${obj.parent_title && obj.parent_objective_id?`<div class="col-12"><div class="detail-sec-title"><i class="bi bi-link-45deg"></i> Parent Objective</div><div class="fw-600 small"><button class="btn btn-link p-0 fw-700 text-start" style="font-size:.88rem;color:var(--primary);text-decoration:none;border:none;background:none" onclick="openDetail(${obj.parent_objective_id})" title="Open parent objective"><i class="bi bi-box-arrow-up-right me-1" style="font-size:.75rem"></i>${obj.parent_title}</button></div></div>`:'' }
    </div>
...
function submitKR(){
  ...
  const title = document.getElementById('krTitle');
  const desc  = document.getElementById('krDesc');
  const unit  = document.getElementById('krUnit');
  if (title && title.value.trim().length === 0) {
    title.setCustomValidity('Title is required.');
  } else if (title && title.value.length > 255) {
    title.setCustomValidity('Title cannot exceed 255 characters.');
  } else if (title) {
    title.setCustomValidity('');
  }
  if (desc && desc.value.length > 500) {
    desc.setCustomValidity('Description cannot exceed 500 characters.');
  } else if (desc) {
    desc.setCustomValidity('');
  }
  if (unit && unit.value.length > 50) {
    unit.setCustomValidity('Unit cannot exceed 50 characters.');
  } else if (unit) {
    unit.setCustomValidity('');
  }

  // Starting value must not exceed target value
  const current = parseFloat(document.getElementById('krCurrent')?.value || 0);
  const target  = parseFloat(document.getElementById('krTarget')?.value  || 0);
  const krTgt   = document.getElementById('krTarget');
  if (target <= 0) {
    krTgt?.setCustomValidity('Target value must be greater than 0.');
  } else if (current > target) {
    krTgt?.setCustomValidity('Starting value cannot exceed the target value.');
  } else {
    krTgt?.setCustomValidity('');
  }
  ...
  fetch('php/kr_api.php',{method:'POST',body:new FormData(form)}).then(r=>r.json()).then(res=>{
    if(res.success){
      // Reset char counters before hiding
      ['krTitleCount','krDescCount','krUnitCount'].forEach(id => {
        const el = document.getElementById(id); if (el) el.textContent = '0 / '+el.textContent.split('/')[1].trim();
      });
      toast('Key result saved!');
...
function updateProgPreview() {
  ...
  // Enforce: new value must not exceed target value
  if (v > progTarget) {
    input.classList.add('is-invalid');
    if (feedback) feedback.textContent =
      `Value cannot exceed the target (${progTarget}). Enter ${progTarget} or less.`;
    input.setCustomValidity(`Cannot exceed target value of ${progTarget}.`);
    // Still show the bar at 100% so user understands where they are
  ...
  // Update progress bar colour based on progress level
...
function submitProgress() {
  ...
  // Block submit if value exceeds target
  if (v > progTarget) {
    input?.setCustomValidity(`Cannot exceed target value of ${progTarget}.`);
    form.reportValidity();
    return;
...
/**
 * charCount(inputId, counterId, max)
 * Updates the character counter label and colours it red when near/at limit.
 */
function charCount(inputId, counterId, max) {
  ...
  // Set/clear browser custom validity so form.reportValidity() works
  if (len > max) {
    el.setCustomValidity(`Cannot exceed ${max} characters.`);
  } else {
...
/**
 * validateKRValues()
 * Ensures starting value ≤ target value in the KR form.
 */
function validateKRValues() {
  const curEl = document.getElementById('krCurrent');
  const tgtEl = document.getElementById('krTarget');
  if (!curEl || !tgtEl) return;
  const cur = parseFloat(curEl.value) || 0;
  const tgt = parseFloat(tgtEl.value) || 0;
  if (tgt <= 0) {
    tgtEl.setCustomValidity('Target must be greater than 0.');
  } else if (cur > tgt) {
    tgtEl.setCustomValidity('Starting value cannot exceed the target value.');
  } else {
    tgtEl.setCustomValidity('');
...
function filterAndPage() {
  ...
  if (countEl) {
    ...
    countEl.textContent = q
      ? `${visible.length} of ${total} objective${total !== 1 ? 's' : ''}`
  ...
  visible.slice(start, start + PAGE_SIZE).forEach(card => card.style.display = '');
...
function renderPagination(visibleCount, totalPages) {
  ...
  const end   = Math.min(currentPage * PAGE_SIZE, visibleCount);
  if (info) info.textContent = `Showing ${start}–${end} of ${visibleCount}`;

  // Build page number buttons
  let html = `<li class="page-item ${currentPage === 1 ? 'disabled' : ''}">
    <button class="page-link" onclick="goPage(${currentPage - 1})">
  ...
  let hi = Math.min(totalPages, currentPage + range);
  ...
  if (hi < totalPages) html += `<li class="page-item disabled"><span class="page-link">…</span></li>`;
...
function goPage(p) {
  const q = (document.getElementById('objSearch')?.value || '').toLowerCase().trim();
  const cards   = Array.from(document.querySelectorAll('#objList .okr-card'));
  const visible = cards.filter(c => !q || c.textContent.toLowerCase().includes(q));
  ...
  document.getElementById('objList')?.scrollIntoView({behavior:'smooth', block:'start'});
}

// Run on load so pagination appears correctly from the start
document.addEventListener('DOMContentLoaded', filterAndPage);
</script>
</body></html>
