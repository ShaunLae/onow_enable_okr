...
<!-- Edit KR Modal (Manager OR KR creator) -->
<?php
// Show edit modal if ANY visible KR is modifiable by this user
$anyCanModify = !empty(array_filter($allKRs, fn($k) => $k['can_modify']));
?>
<?php if ($anyCanModify): ?>
    ...
    <div class="modal-body">
      <form id="editKrForm">
        ...
        <div class="row g-3">
          ...
          <?php if ($role==='Manager'): ?>
          <div class="col-12"><label class="form-label">Owner</label><select name="owner_id" id="ekOwner" class="form-select"></select><div class="form-text">Only members assigned to this objective.</div></div>
          <?php else: ?>
          <input type="hidden" name="owner_id" id="ekOwner" value="<?= $userId ?>">
          <?php endif; ?>
        </div>
      </form>
    </div>
...
function editKR(id,kr){
  ...
  // Managers get the owner dropdown populated via AJAX.
  // Members editing their own created KR use a hidden field (no reassignment).
  const ownerSel = document.getElementById('ekOwner');
  if (ROLE === 'Manager' && ownerSel && ownerSel.tagName === 'SELECT') {
    fetch(`php/kr_api.php?obj_members=1&objective_id=${kr.objective_id}`).then(r=>r.json()).then(ms=>{
      ownerSel.innerHTML=`<option value="${ME}"><?= htmlspecialchars($user['full_name']) ?> (You)</option>`;
      ms.forEach(m=>{if(m.id!=ME)ownerSel.innerHTML+=`<option value="${m.id}">${m.full_name} (${m.role})</option>`;});
      ownerSel.value=kr.owner_id;
    });
  }
