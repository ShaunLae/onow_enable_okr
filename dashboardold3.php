...
// ─── Quarter filter ──────────────────────────────────────────────
// Reads ?q=Q1+2026 from the URL. Builds two WHERE-clause fragments:
//   $qObjWhere   applies to objectives via time_period column (string match)
//   $qPhWhere    applies to progress_history via created_at date range
//   $qDateWhere  applies to objectives via start_date/end_date overlap
$availableQuarters = getTimePeriods();   // from functions.php
$selectedQ = isset($_GET['q']) ? trim($_GET['q']) : '';
// Validate: must be one of the known periods or empty
if ($selectedQ && !in_array($selectedQ, $availableQuarters)) $selectedQ = '';

// Build reusable SQL snippets + bound params for the quarter filter
$qObjClause  = '';   // "AND o.time_period = :qp"
$qBaseClause = '';   // "AND time_period   = :qp"  (when alias is not used)
$qPhClause   = '';   // "AND ph.created_at BETWEEN :qs AND :qe"
$qParams     = [];   // extra PDO params to merge into every execute()

if ($selectedQ !== '') {
    // Parse "Q1 2026" → date range Jan 1 – Mar 31
    [$qPart, $qYear] = explode(' ', $selectedQ);
    $qYear = (int)$qYear;
    ...
    $qPhClause   = " AND ph.created_at BETWEEN :qs AND :qe";
    $qParams     = [':qp' => $selectedQ, ':qs' => $qStart, ':qe' => $qEnd.' 23:59:59'];
}
...
// ════════════════════════════════════════════════════════════════
// MEMBER — 6 standalone queries  (all respect $selectedQ)
// ════════════════════════════════════════════════════════════════
if ($role === 'Member') {

    // M-Q1: My Key Results — filtered via parent objective's time_period
    $stmt = $db->prepare("
    ...
    ");
    $stmt->execute(array_merge([':uid' => $userId], $qParams));
    $myKRs = $stmt->fetchAll();

    // M-Q2: My progress updates — filtered by created_at within quarter date range
    $stmt = $db->prepare("
        ...
        " . ($selectedQ ? " AND ph.created_at BETWEEN :qs AND :qe"
                        : " AND ph.created_at >= DATE_SUB(NOW(), INTERVAL 7 DAY)") . "
    ");
    $phP2 = [':uid' => $userId];
    if ($selectedQ) { $phP2[':qs'] = $qParams[':qs']; $phP2[':qe'] = $qParams[':qe']; }
    $stmt->execute($phP2);
    $weekActivity = $stmt->fetch();
    $weekLabel = $selectedQ ? "In $selectedQ" : "This week";

    // M-Q3: Objectives I am contributing to — filtered by time_period
    $stmt = $db->prepare("
    ...
    ");
    $stmt->execute(array_merge([':uid' => $userId, ':uid2' => $userId], $qParams));
    $contributingTo = $stmt->fetchAll();

    // M-Q4: Upcoming deadlines — filtered by time_period
    $stmt = $db->prepare("
    ...
    // M-Q5: My completion rate — filtered by time_period
    $stmt = $db->prepare("
    ...
    // M-Q6: Recent progress I submitted — filtered by created_at in quarter
    $stmt = $db->prepare("
...
// ════════════════════════════════════════════════════════════════
// MANAGER — 6 standalone queries  (all respect $selectedQ)
// ════════════════════════════════════════════════════════════════
if ($role === 'Manager') {

    // MG-Q1: Team health — filtered by objective time_period
    $stmt = $db->prepare("
        ...
        LEFT   JOIN objectives o ON (
               ...
               AND o.deleted_at IS NULL
               " . ($selectedQ ? "AND o.time_period = :qp" : "") . "
    ...
    // MG-Q2: Objectives by type — filtered by time_period
    $stmt = $db->prepare("
        SELECT type,
               COUNT(*)      AS obj_count,
    ...
    // MG-Q3: Key results needing attention — filtered by objective time_period
    $stmt = $db->prepare("
        SELECT kr.id, kr.title, kr.progress, kr.status,
    ...
    // MG-Q4: Team progress feed — filtered by progress_history created_at
    $stmt = $db->prepare("
    ...
    // MG-Q5: At-risk deadlines — filtered by time_period
    $stmt = $db->prepare("
    ...
    // MG-Q6: Parent vs child alignment — filtered by time_period on child objective
    $stmt = $db->prepare("
...
<main class="main-content">
  ...
  <?php elseif ($role === 'Member'): ?>
    ...
    <?php foreach ([
      ...
      ['Completion',  $completionPct.'%',          'bi-trophy',              'var(--secondary)', '#ede9fe',           ($completionRate['completed']??0).' of '.($completionRate['total']??0).' owned'],
    ...
    <div class="row g-3">
      <div class="col-lg-8"><div class="card h-100">
        <div class="card-header"><i class="bi bi-bar-chart-line text-primary"></i><span class="card-title">Objectives Overview<?= $selectedQ?" — $selectedQ":'' ?></span>
          <select class="form-select form-select-sm ms-auto no-print" id="chartFilter" style="width:auto" onchange="renderChart(this.value)">
            <option value="all">All Types</option><option value="Personal">Personal</option>
          </select>
        </div>
        <div class="card-body">
        <?php if (empty($objectives)): ?>
          <div class="empty-state"><i class="bi bi-bar-chart"></i><h5>No Objectives<?= $selectedQ?" for $selectedQ":'' ?></h5><a href="objectives.php" class="btn btn-primary btn-sm mt-2 no-print">Get Started</a></div>
  ...
  <?php else: ?>
    ...
    <div class="row g-3 mb-3">
      ...
      <div class="col-lg-6"><div class="card h-100">
        <div class="card-header"><i class="bi bi-bar-chart-line text-primary"></i><span class="card-title">Objectives Overview<?= $selectedQ?" — $selectedQ":'' ?></span>
          <select class="form-select form-select-sm ms-auto no-print" id="chartFilter" style="width:auto" onchange="renderChart(this.value)">
            <option value="all">All Types</option><option value="Organisational">Organisational</option><option value="Team">Team</option><option value="Personal">Personal</option>
          </select>
        </div>
        <div class="card-body">
        <?php if (empty($objectives)): ?>
...
new Chart(document.getElementById('statusChart'), {
    type: 'doughnut',
    data: { labels: ['On Track','At Risk','Completed','Not Started'], datasets: [{ data: [<?= $stats['on_track'] ?>,<?= $stats['at_risk'] ?>,<?= $stats['completed'] ?>,<?= $stats['not_started'] ?>], backgroundColor: ['#059669','#d97706','#3b82f6','#e2e8f0'], borderWidth: 2, borderColor: '#fff' }] },
