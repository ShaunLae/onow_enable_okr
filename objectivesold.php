        ...
        <div class="kr-item">
          <div class="d-flex align-items-start justify-content-between gap-2">
            <div class="flex-grow-1">
              ...
              <div class="kr-values"><?= number_format($kr['current_value'],1) ?> / <?= number_format($kr['target_value'],1) ?> <?= htmlspecialchars($kr['unit']) ?> · <?= $kr['metric_type'] ?> · <span class="text-muted">Owner: <?= htmlspecialchars($kr['owner_name']) ?></span></div>
            ...
            <div class="d-flex gap-1 align-items-start flex-wrap">
              ...
              <?php if ($role==='Manager'): ?>
              <button class="btn btn-sm btn-outline-secondary btn-icon" onclick="editKR(<?= $kr['id'] ?>,<?= htmlspecialchars(json_encode($kr)) ?>)"><i class="bi bi-gear"></i></button>
              <button class="btn btn-sm btn-outline-danger btn-icon" onclick="deleteKR(<?= $kr['id'] ?>,'<?= htmlspecialchars(addslashes($kr['title'])) ?>')"><i class="bi bi-trash"></i></button>
              <?php endif; ?>
            </div>
          </div>
        </div>
