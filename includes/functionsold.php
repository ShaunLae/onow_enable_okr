...
/* ══════════════════════════════════════════════════
   STATUS HELPERS
══════════════════════════════════════════════════ */
function getProgressStatus(float $p, ?string $end=null): string {
    if ($p >= 100) return 'Completed';
    $d = $end ? (strtotime($end)-time())/86400 : null;
    if ($p >= 70) return 'On Track';
    if ($p >= 40) return ($d!==null && $d<14) ? 'At Risk' : 'On Track';
...
function getObjectives(int $userId, string $role, ?string $type=null, ?string $period=null, ?int $teamId=null): array {
    $db = getDB(); $params = [];
    $base = "SELECT o.*,u.full_name as owner_name,c.full_name as created_by_name,po.title as parent_title,t.name as team_name FROM objectives o LEFT JOIN users u ON o.owner_id=u.id LEFT JOIN users c ON o.created_by=c.id LEFT JOIN objectives po ON o.parent_objective_id=po.id LEFT JOIN teams t ON o.team_id=t.id LEFT JOIN objective_members om ON o.id=om.objective_id";

    if ($role==='Manager') {
        $sql = "$base WHERE o.deleted_at IS NULL AND(o.created_by=? OR om.user_id=? OR o.type='Organisational')";
        $params = [$userId,$userId];
    } else {
        $sql = "$base WHERE o.deleted_at IS NULL AND(o.owner_id=? OR om.user_id=? OR o.type='Organisational')";
        $params = [$userId,$userId];
    }
...
function getObjective(int $id): ?array {
    $s = getDB()->prepare("SELECT o.*,u.full_name as owner_name,c.full_name as created_by_name,po.title as parent_title,t.name as team_name FROM objectives o LEFT JOIN users u ON o.owner_id=u.id LEFT JOIN users c ON o.created_by=c.id LEFT JOIN objectives po ON o.parent_objective_id=po.id LEFT JOIN teams t ON o.team_id=t.id WHERE o.id=? AND o.deleted_at IS NULL");
...
function getParentOptions(int $userId, string $role, ?int $excludeId=null): array {
    $db = getDB();
    $sql = $role==='Manager'
        ? "SELECT o.id,o.title,o.type,o.time_period FROM objectives o LEFT JOIN objective_members om ON o.id=om.objective_id WHERE o.deleted_at IS NULL AND(o.created_by=? OR om.user_id=? OR o.type='Organisational')"
        : "SELECT o.id,o.title,o.type,o.time_period FROM objectives o LEFT JOIN objective_members om ON o.id=om.objective_id WHERE o.deleted_at IS NULL AND(o.owner_id=? OR om.user_id=? OR o.type='Organisational')";
    $p = [$userId,$userId];
...
function createObjective(array $d, int $userId): array {
    $db = getDB();
    try {
        $s = $db->prepare("INSERT INTO objectives(title,description,type,status,time_period,start_date,end_date,owner_id,created_by,parent_objective_id,team_id,progress) VALUES(?,?,?,'Not Started',?,?,?,?,?,?,?,0.00)");
        $s->execute([
            sanitize($d['title']),
...
function updateObjective(int $id, array $d, int $userId): array {
    $db = getDB();
    try {
        $db->prepare("UPDATE objectives SET title=?,description=?,type=?,time_period=?,start_date=?,end_date=?,owner_id=?,parent_objective_id=?,team_id=?,updated_at=NOW() WHERE id=?")->execute([
            sanitize($d['title']),
            sanitize($d['description']??''),
            $d['type'],
            sanitize($d['time_period']),
...
function getKeyResults(int $objectiveId): array {
    $s = getDB()->prepare("SELECT kr.*,u.full_name as owner_name FROM key_results kr LEFT JOIN users u ON kr.owner_id=u.id WHERE kr.objective_id=? AND kr.deleted_at IS NULL ORDER BY kr.created_at ASC");
    $s->execute([$objectiveId]); return $s->fetchAll();
}

function getKeyResult(int $id): ?array {
    $s = getDB()->prepare("SELECT kr.*,u.full_name as owner_name FROM key_results kr LEFT JOIN users u ON kr.owner_id=u.id WHERE kr.id=? AND kr.deleted_at IS NULL");
    $s->execute([$id]); return $s->fetch()?:null;
}
...
function createKeyResult(array $d, int $userId): array {
    ...
    try {
        ...
        $db->prepare("INSERT INTO key_results(objective_id,title,description,metric_type,target_value,current_value,unit,progress,status,owner_id) VALUES(?,?,?,?,?,?,?,?,?,?)")->execute([
            ...
            sanitize($d['unit']??''),$p,getProgressStatus($p),(int)($d['owner_id']??$userId),
        ]);
        $kid = $db->lastInsertId();
...
function updateKeyResult(int $id, array $d, int $userId): array {
    ...
    try {
        ...
        $db->prepare("UPDATE key_results SET title=?,description=?,metric_type=?,target_value=?,current_value=?,unit=?,progress=?,status=?,owner_id=?,updated_at=NOW() WHERE id=?")->execute([
            ...
            $p,getProgressStatus($p),(int)($d['owner_id']??$userId),$id,
        ]);
...
function recalcObjective(int $objectiveId): void {
    ...
    $status = getProgressStatus($p,$obj['end_date']??null);
